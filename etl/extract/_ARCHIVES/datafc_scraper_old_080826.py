...
LEAGUES = {
    ...
    "BEL": {"tournament_id": 11,  "nom": "First Division A"},
...
SEASON_IDS = {
    ...
    11:  {"23/24": None,  "24/25": None,  "25/26": None,  "22/23": None},  # À corriger
    ...
    for metric in METRICS:
        try:
            ...
            if df is not None and not df.empty:
                df["_metric_order"] = metric
                all_dfs.append(df)
                log.info(f"    {metric} → {len(df)} joueurs")
            else:
                log.warning(f"    {metric} → vide")

        except datafc.RateLimitError:
            log.warning(f"    {metric} → Rate limit, pause 30s...")
            time.sleep(30)
        except datafc.DataNotAvailableError:
            log.warning(f"    {metric} → Non disponible")
        except Exception as e:
            log.error(f"    {metric} → {e}")
